...
public class ArticleCourseImpl implements ArticleCourseDao {
	...
	public Boolean DeleteById(String id) {
		String sql = "UPDATE article SET article.is_delete = 1 WHERE id = ?;";
	...
	public ArrayList<ArticleCourse> getUnreviewedArticle(Integer index, Integer limit, String str) {
		String sql = "SELECT a.id, a.article_name, a.article_type, a.article_content, c.id courseId,"
				+ " c.course_name, u.id userId, u.user_name, a.check_state, a.create_time, a.finish_time,"
				+ " a.is_delete, a.remark FROM article a, `user` u, course c, course_article ca WHERE c.id = ca.course_id AND"
				+ " ca.article_id = a.id AND u.id = a.auther_id AND a.is_delete = 0 AND check_state = 1"
				+ " AND a.article_name LIKE '%" + StringUtil.emptyOrNull(str) + "%'LIMIT " + index + "," + limit + ";";
		ArrayList<ArticleCourse> acList = new ArrayList<ArticleCourse>();
		Connection connection = DBUtil.open();
		try{
			PreparedStatement pstm = connection.prepareStatement(sql);
	...
	public Integer getUnreviewedSize(String str) {
		String sql = "SELECT a.id, a.article_name, a.article_type, a.article_content, c.id courseId,"
				+ " c.course_name, u.id userId, u.user_name, a.check_state, a.create_time, a.finish_time,"
				+ " a.is_delete, a.remark FROM article a, `user` u, course c, course_article ca WHERE c.id = ca.course_id AND "
				+ "ca.article_id = a.id AND u.id = a.auther_id AND a.is_delete = 0 AND check_state = 1 AND a.article_name LIKE '%" 
				+ StringUtil.emptyOrNull(str) + "%';";
		ArrayList<ArticleCourse> acList = new ArrayList<ArticleCourse>();
		Connection connection = DBUtil.open();
		try{
	...
	public Boolean changeCheckState(String id, Byte checkState) {
		String sql = "UPDATE article SET check_state = ?, finish_time = NOW() WHERE id = ?;";
		if(this.isTeacherExist(id) == false){
			return false;
		}
		Connection connection = DBUtil.open();
